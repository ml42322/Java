...
public class NutriProfiler {
	...
	//uses calculateNutriRequirement() and calculateEnergyRequirement() to create the 
	//person's nutriProfile
	//adds each recommendednutrient to the recommendedNutrientsList
	static void createNutriProfile(Person person) {
		...
		RecommendedNutrient a = new RecommendedNutrient(ENERGY_NUTRIENT_CODE, energy);
		person.recommendedNutrientsList.add(a);
		NutriEnum nutriEnum = null;
		int counter = 0;
		for(NutriEnum n : nutriEnum.values()){
			RecommendedNutrient b = new RecommendedNutrient(n.getNutrientCode().toString(), nutReq[counter]);
			counter += 1;			
			person.recommendedNutrientsList.add(b);
		}
		}
